...
def convert_object(mask, image):
    resized = cv2.resize(mask, (280, 497), interpolation = cv2.INTER_AREA)
    gray = resized
    ...
    resized_image = cv2.resize(image, (280, 497), interpolation = cv2.INTER_AREA)
    warped = four_point_transform(resized_image, pts)
    return warped
